...
def simulate():
    ...
    for i in range(m):
        ...
        # if curr_t > 37:
        print(f"Person {i} ==> {px}, {py} => {min_x}, {min_y} {min_dist}_{idx}")
    ...
    for i in range(m):
        if people[i] == cvs_list[i]:
            px, py = people[i]
            # if grid[px][py] != 2:
              #   print(f"Updated : {px}, {py} Time : {curr_t}")
            grid[px][py] = 2
    ...
    # print(f"Moved to camp : {min_x},{min_y}") ## 베이스 캠프 이동하는 부분까지는 잘 맞았음.
